...
| **MCP Server**  | A program that provides context, tools, or resources to the client.   | The `arxiv_server.py` script we will build.               |
...
Now we will walk through the `arxiv_server.py` file, explaining each section in detail. The full, commented code is available in the project directory.
...
async def make_arxiv_request(url: str) -> Optional[str]:
    # ... (Implementation in arxiv_server.py)
...
2.  Add an entry for your server, making sure to use the **absolute path** to your `arxiv_server.py` script.
...
```json
{
  "mcpServers": {
    "arxiv-search": {
      ...
      "args": [
        "/absolute/path/to/your/project/arxiv_server.py"
      ],
      "env": {
