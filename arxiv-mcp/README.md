...
```json
{
  "mcpServers": {
    "arxiv-search": {
      "command": "python3",
      "args": [
        "/absolute/path/to/arxiv_server.py"
      ],
      "env": {
...
1. **arxiv_server.py**: Direct API calls using `httpx`
2. **arxiv_server_sdk.py**: Using official OpenAI and Anthropic SDKs
...
2. Verify the absolute path to `arxiv_server.py` is correct
